...
from localweid import generate_addr
...
class Base(object):
    ...
    def get(self, url, params=""):
        ...
        response = requests.get("{BASEURL}{url}".format(
            BASEURL=self.BASEURL, url=url), params=params)
        if response.status_code >= 400:
            LOG.warning('create charging_rule error: %s:%s',
    ...
    def post(self, url, data):
        ...
        response = requests.post("{BASEURL}{url}".format(
            BASEURL=self.BASEURL, url=url), json=data)
        if response.status_code >= 400:
            LOG.warning('create charging_rule error: %s:%s',
                        response.status_code, response.text)
            return None
    ...
    def ecdsa_sign(self, encode_transaction, privkey, hashfunc=hashlib.sha256):
        ...
        signning_key = SigningKey.from_string(
            bytes.fromhex(privkey), curve=SECP256k1)
